...
@callback(
    Output(component_id='traditional-supply-chain-path', component_property='children'),
...
def update_input_data_path(cloud_setting):
    ...
    input_data_path = f'customers/{customer_name}/apni/traditionalSupplyChain/TR/'
    logger.info(f'\tCurrent input_data_path={input_data_path}')

    layout = html.Div(
        children=[
            get_historical_data_path_layout(historical_data_path),
            get_input_data_path_layout('Traditional Results Path:', input_data_path),
        ],
    )
